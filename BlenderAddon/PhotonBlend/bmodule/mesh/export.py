...
def _export_original_mesh_object_v3p6(b_mesh_object: bpy.types.Object, console: SdlConsole):
    """
    Export Blender original mesh object. This will group faces with the same material, then export each
    material-faces pair as a Photon actor. This version manually loops over each triangle and saves all
    data in raw SDL, which can be slow and produce large files for larger scenes. Nevertheless, this is
    a good reference as it handles everything explicitly and lives through Blender 2 ~ 3.
    """
    b_mesh = b_mesh_object.data
    b_mesh.calc_loop_triangles()
...
def _export_original_mesh_object(b_mesh_object: bpy.types.Object, console: SdlConsole):
    ...
    if bpy.app.version >= (4, 5, 0):
        _export_original_mesh_object_v4p5(b_mesh_object, console)
    elif bpy.app.version >= (3, 6, 0) and bpy.app.version < (4, 1, 0):
        _export_original_mesh_object_v3p6(b_mesh_object, console)
    else:
        # It is known that 4.1.0 changed now normal is handled, this will fail likely
        # (https://docs.blender.org/api/4.1/change_log.html#id29)
        _export_original_mesh_object_v3p6(b_mesh_object, console)

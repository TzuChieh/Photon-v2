...
def is_masked(b_material: bpy.types.Material):
    ...
    # Check whether the interface mask socket is connected
    return output_node.inputs["Interface Mask"].is_linked
...
def get_interface_mask_image_res_name(b_material: bpy.types.Material):
    """
    @return Name of the SDL image resource associated with the interface mask socket.
    `None` if `is_masked()` is `False`.
    """
    output_node = find_output_node_from_material(b_material)
    return output_node.get_interface_mask_res_name(b_material) if output_node is not None else None

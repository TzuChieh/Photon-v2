#include "Engine/Actor/Geometry/GWave.h"
	...
	CookedGeometry& out_geometry) const
{
	...
	for(int32 iz = 0; iz < numZdivs; iz++)
	{
		for(int32 ix = 0; ix < numXdivs; ix++)
		{
			...
			PTriangle tri1(vA, vB, vC);
			tri1.setNa(nA);
			tri1.setNb(nB);
			tri1.setNc(nC);

			PTriangle tri2(vA, vC, vD);
			tri2.setNa(nA);
			tri2.setNb(nC);
			tri2.setNc(nD);
	...
	// Wave bottom rectangle
	{
		...
		PTriangle tri1(vA, vD, vB);
		PTriangle tri2(vC, vB, vD);
	...
	for(int32 ix = 0; ix < numXdivs; ix++)
	{
		...
		PTriangle tri1(vA, vC, vB);
		PTriangle tri2(vA, vD, vC);
...
bool GWave::checkData(const real xLen, const real yLen, const real zLen)
{

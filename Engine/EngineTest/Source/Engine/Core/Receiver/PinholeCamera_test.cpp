...
TEST(PinholeCameraTest, ReportsRaySampleDims)
{
	const PinholeCamera staticCamera(
		math::Vector2D(1, 1),
		&StaticAffineTransform::IDENTITY(),
		&StaticRigidTransform::IDENTITY(),
		TimeStep());

	const PinholeCamera motionCamera(
		math::Vector2D(1, 1),
		&StaticAffineTransform::IDENTITY(),
		&StaticRigidTransform::IDENTITY(),
...
TEST(PinholeCameraTest, SamplesRayTimeInStep)
{

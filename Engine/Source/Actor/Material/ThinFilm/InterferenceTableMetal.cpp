...
// #include "InterferenceTableMetal.h"
#include <fstream>
#include <sstream>
#include <string>
#include <iostream>
...
namespace ph
{
    ...
    InterferenceTableMetal::InterferenceTableMetal() = default;
    InterferenceTableMetal::InterferenceTableMetal(const std::vector<complex_f> ior, const std::vector<float> thickness)
    {
        ior_vec = ior;
        thickness_vec = thickness;
    }

    void InterferenceTableMetal::write_to_vec(const float angle, const float wl, const float R, const float T)
    {
    ...
    // void InterferenceTableMetal::interpolate(float x1, float x2, float y1, float y2, float x)
    // {
    //     assert(x1 != x2);
    ...
    void InterferenceTableMetal::output_log()
    {
		std::ofstream f_log("metal.log");
		if (f_log.is_open())
		{
    ...
    void InterferenceTableMetal::output_result()
    {
    ...
    void InterferenceTableMetal::simulate_single_thin_film()
    {
    ...
    std::vector<float> InterferenceTableMetal::calc_recur_R(const complex_f n1, const complex_f n2, const complex_f n3, const complex_f theta_1, const complex_f theta_2, const complex_f theta_3, const int wl, const int d)
    {
    ...
    complex_f InterferenceTableMetal::calc_rs(const complex_f n1, const complex_f n2, const complex_f theta_i, const complex_f theta_t)
    {
    ...
    complex_f InterferenceTableMetal::calc_rp(const complex_f n1, const complex_f n2, const complex_f theta_i, const complex_f theta_t)
    {
    	if (theta_i == complex_f(90,0)) return -1;
    	if (theta_t == complex_f(90,0)) return 1;

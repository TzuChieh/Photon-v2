...
void StandardSampleGenerator::analyze(const World& world, const Film& film)
{
	m_filmWidthPx = film.getWidthPx();
	m_filmHeightPx = film.getHeightPx();
}

void StandardSampleGenerator::requestMoreSamples(std::vector<Sample>* const out_samples)
{
	if(!hasMoreSamples())
	{
		return;
...
void StandardSampleGenerator::split(const uint32 nSplits, std::vector<std::unique_ptr<SampleGenerator>>* const out_sampleGenerators)
{
	...
	{
		auto sampleGenerator = std::make_unique<StandardSampleGenerator>(splittedSppBudget);

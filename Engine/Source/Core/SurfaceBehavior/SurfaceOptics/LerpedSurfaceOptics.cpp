	...
	SpectralStrength* const out_bsdf) const
{
	...
	m_optics0->evalBsdf(X, L, V, &bsdf0);
	m_optics1->evalBsdf(X, L, V, &bsdf1);
	...
	SpectralStrength* const out_pdfAppliedBsdf) const
{
	...
	if(dart < prob)
	{
		...
		m_optics0->genBsdfSample(X, V, out_L, &pdfAppliedBsdf0);
		m_optics1->evalBsdf(X, *out_L, V, &bsdf1);

		real pdfW0, pdfW1;
		m_optics0->calcBsdfSamplePdf(X, *out_L, V, &pdfW0);
		m_optics1->calcBsdfSamplePdf(X, *out_L, V, &pdfW1);
	...
	else
	{
		...
		m_optics1->genBsdfSample(X, V, out_L, &pdfAppliedBsdf1);
		m_optics0->evalBsdf(X, *out_L, V, &bsdf0);
...
{
	// FIXME: types are not the original ones generated by optics 0 and 1

	const SpectralStrength ratio = m_sampler.sample(*m_ratio, X);
	const real             prob  = pickOptics0Probability(ratio);

	real pdf0, pdf1;
	m_optics0->calcBsdfSamplePdf(X, L, V, &pdf0);
	m_optics1->calcBsdfSamplePdf(X, L, V, &pdf1);

...
#include "Math/Color/TColorBase.h"
...
template<typename U>
inline TColorBase<Derived, T, N>::TColorBase(const std::array<U, N>& values)
{
...
template<typename Derived, typename T, std::size_t N>
template<typename U>
inline TColorBase<Derived, T, N>::TColorBase(const TArithmeticArray<U, N>& values)
{
	for(std::size_t i = 0; i < N; ++i)
	{
		m[i] = static_cast<T>(values[i]);
...
template<typename Derived, typename T, std::size_t N>
template<typename U>
inline TColorBase<Derived, T, N>::TColorBase(const U* const values)
{
	PH_ASSERT(values);
...
template<typename Derived, typename T, std::size_t N>
template<typename U>
inline TColorBase<Derived, T, N>::TColorBase(const std::vector<U>& values)
{

...
#include "Model/Geometry/DifferentialPatch.h"
...
void Triangle::calcDifferentialPatch(const Vector3f& hitPosition, DifferentialPatch* const out_differentialPatch) const
{
	//Vector3f localHitPoint; 
	//m_parentModel->getWorldToModelTransform()->transformPoint(hitPosition, &localHitPoint);
	...
	//// find dominant axis
	//if(fabs(m_faceNormal.x) > fabs(m_faceNormal.y))
	//{
	//	// X dominant, projection plane is YZ
	//	if(fabs(m_faceNormal.x) > fabs(m_faceNormal.z))
	//	{
	...
	//}
	//// Y dominant, projection plane is ZX
	//else if(fabs(m_faceNormal.y) > fabs(m_faceNormal.z))
	//{
	...
	//float32 multiplier = 1.0f / (abPu*acPv - abPv*acPu);

	//// barycentric coordinate of vertex B in the projected plane
	//float32 baryB = (hitPu*acPv - hitPv*acPu) * multiplier;

	//// barycentric coordinate of vertex C in the projected plane
	//float32 baryC = (hitPv*abPu - hitPu*abPv) * multiplier;

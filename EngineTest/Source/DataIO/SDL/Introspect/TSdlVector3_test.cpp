...
{
	{
		...
		EXPECT_NO_THROW(sdlVec.fromSdl(owner, SdlPayload("1 2 3"), ctx));

		EXPECT_EQ(owner.v, Vector3R(1, 2, 3));

		// Spaces in SDL string are skipped
		EXPECT_NO_THROW(sdlVec.fromSdl(owner, SdlPayload("  1 1   1 "), ctx));
		EXPECT_EQ(owner.v, Vector3R(1, 1, 1));

		// Read string with different importance

		sdlVec.optional();
		EXPECT_NO_THROW(sdlVec.fromSdl(owner, SdlPayload("0 0 1"), ctx));
		...
		sdlVec.required();
		EXPECT_NO_THROW(sdlVec.fromSdl(owner, SdlPayload("100 100 -200"), ctx));

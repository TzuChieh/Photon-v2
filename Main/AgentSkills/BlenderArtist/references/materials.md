...
3. Enumerate active-scene objects whose material slots reference the exact material datablock, then record each slot's actual polygon use. Do not use `Material.users` as an object or face count: linked mesh instances share datablock references, non-object users can contribute, and an assigned slot can have zero faces. Use evaluated geometry when modifiers can change topology or material indices.
4. Record object and mesh names, linked instances, slot indices, face counts, geometry role, UV domain, and visible-side intent when the graph alone is ambiguous; topology alone does not establish meaning.
5. Treat imported Cycles, Principled, or legacy FBX nodes as evidence, not authority, when they conflict with authoritative source data or observable source behavior.
6. Derive channel packing, alpha meaning, normal handedness, and color spaces from authoritative metadata. Do not assume a universal ORM layout, opacity-in-alpha convention, or DirectX/OpenGL normal format; OpenPBR does not define those texture encodings.
7. Once channel semantics are verified, treat every nonconstant authored value, including slight variation, as mapped.
8. Transform color-managed inputs from their declared source color space into the active working color space. Keep data explicitly authored as scalar or vector raw; a scalar destination alone does not make a color image raw.
9. Compare every effective source parameter with the installed Photon node inputs. Record the source expression and its target result as exact, qualitative approximation, or unsupported; do not describe a partial mapping as complete.
...
- Rough dielectric interface: use Abraded Translucent for exact or Schlick Fresnel, IOR, and mapped isotropic or anisotropic roughness. It has no thickness, absorption, scattering, or tint inputs. Record spatial transmission tint separately: a future surface-tint input must evaluate a spectrum image per hit and modulate only transmission. Do not conflate it with Beer-Lambert absorption, which needs an absorption coefficient and traveled thickness.
...
- Image Transform can wrap color or numeric image resources, including procedural outputs and opacity-mask graphs. It cannot choose a coordinate domain, reconstruct a missing coordinate component, invert a non-affine UV unwrap, or choose different transforms per object for a shared material. Use Attribute for its supported non-UV domains.
- Use Attribute `UVW From Geometry Bound` for local hit position normalized by the complete local geometry AABB; a zero-extent axis returns `0.5`. Map it to source Generated coordinates only after verifying equivalent bounds semantics.
- Use Attribute `Geometry Hit Position` for unnormalized local-space hit position and connect it to Noise's `Coordinates` socket for matching Object/local-position paths. Attribute does not expose arbitrary named objects or attributes; report unsupported domains rather than compensating with Noise frequency.
...
- When an audit compares winding-derived `Ng` with the normalized average of three supplied corner normals, report a face-averaged shading-normal mismatch; do not claim that every corner normal is reversed. Include every linked object instance, since determinant parity and a local normal mismatch are independent.
...
- Picture and conversion nodes: `BlenderAddon/PhotonBlend/bmodule/material/input_nodes/picture.py`, `BlenderAddon/PhotonBlend/bmodule/material/conversion_nodes/split_image.py`, `luminance.py`, `hsv.py`, `color_remap.py`, and the corresponding `Engine/Engine/Source/Engine/Actor/Image` implementations
- Coordinate inputs and transforms: `BlenderAddon/PhotonBlend/bmodule/material/input_nodes/attribute.py`, `BlenderAddon/PhotonBlend/bmodule/material/conversion_nodes/transformed_image.py`, `Engine/Engine/Source/Engine/Actor/Image/AttributeImage.cpp`, and `TransformedImage.cpp`

...
- Map Abraded Opaque F0, roughness, and Reflection Scale. Reflection Scale multiplies the evaluated Fresnel/microfacet lobe after Fresnel and changes neither its angular law nor sampling PDF. With Schlick Fresnel, `F0 = 1` makes `F(theta) = 1`, so preserve the source F0 instead of using 1 as a dielectric boost. Treat Reflection Scale as an artistic, non-physical control.
...
- For a constant glossy-over-diffuse fallback, derive scalar `f = ((IOR - 1) / (IOR + 1))^2 * (2 * Specular IOR Level)`. For `0 < f = F0`, putting `Abraded Opaque(F0=f, Reflection Scale=1/f)` in A, diffuse in B, and `f` in Binary Mix gives reflected contribution `f * (1/f) * F(theta) = F(theta)`. The scale remains constant; the preserved F0 supplies the angle dependence. This leaves diffuse at `(1-f) * C` and glossy selection probability at `f`, so it is not a physical coat, can exceed unit total energy at grazing, and can be noisy for small `f`. Label it qualitative.
- When an exact dielectric-over-diffuse model is unavailable and an approximation is authorized, prefer `C_eff = k * C / (1 - Fdr_int * C)` per color channel over a constant diffuse scale. This sums repeated internal returns `C + Fdr_int*C^2 + ...`, so stronger channels receive a larger relative return and the base becomes more saturated. Derive `Fdr_int` and `k` from source IOR, entry/exit transmission, the target transport convention, and any outer mix weight; never hardcode coefficients from another scene.
- Label a coefficient fitted from camera rays or a screen-space average as camera-matched. Do not use it to compensate for unrelated lighting differences or claim it generalizes to other directions. A physical solution must jointly model angle-dependent coat reflection, lower-layer attenuation, multiple internal reflection, and energy-aware lobe sampling.
...
- Map Abraded Translucent Fresnel, IOR, roughness, and Reflection/Transmission Scale. The scales multiply evaluated lobe energy after Fresnel, while current reflection-versus-transmission selection still uses unscaled Fresnel. Treat them as non-physical per-hit spectral controls; large or strongly colored scales can increase energy and sampling variance.
- Do not equate Transmission Scale with source Base Color, thickness, absorption, scattering, or Beer-Lambert attenuation. A mapped scale compounds whenever a path hits another interface. `sqrt(C)` is only a two-interface heuristic and fails for a single surface, rough or repeated crossings, and topology-dependent paths. Use it only after those semantics are established and a render validates the result; otherwise keep neutral transmission and report the distinct spatial transmission-color behavior as unsupported.
...
- After rejecting an experiment, restore its prior links and values, remove every introduced helper, and repeat the reachability audit; a bypassed helper is stale even when the output looks correct.
- Preserve and restore render settings around diagnostic previews. Treat a low-spp render as export and gross-regression evidence only. Use material/object masks for regional comparisons, and do not tune one material to absolute reference luminance when unrelated lighting or surfaces show broader mismatches.

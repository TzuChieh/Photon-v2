...
[Observer](@ref ph::Observer) plays an important role in a scene: it captures incoming energy and record it on its sensing device called [Film](@ref ph::Film)--so you can observe the virtual world which just got rendered. Observers can generate energy [Receiver](@ref ph::Receiver) and a common example of it is camera, which senses and records lighting as a photograph.
...
If there is a ball in the scene, we can create a [sphere geometry](@ref ph::GSphere) to represent it; if there is a piece of paper on the table, we can use a [rectangle geometry](@ref ph::GRectangle) to model it. Geometries are the bare bones of a scene: materials, lights, physical motions… are all built on top of geometries. We show some common types of geometries in the following sections to give you a rough idea of how geometries are like in Photon renderer.

...
res_dir = paths.test_resources() / "cornell_box_normal_map"
...
pppm_ref_path = res_dir / "ref_pppm_10000passes_beauty"
...
for case_name, output_name, scene_name, ref_path, ref_title, ref_var_path, z_sample_count, max_mse, max_rel_mean in [
    ("BVPT", "bvpt", "scene_bvpt.p2", pt_ref_path, "Reference: BNEEPT 32768 spp", bvpt_ref_var_path, 640, 0.0049, 0.003),
    ("BNEEPT", "bneept", "scene_bneept.p2", pt_ref_path, "Reference: BNEEPT 32768 spp", bneept_ref_var_path, 256, 0.00165, 0.003),
    ("PPPM", "pppm", "scene_pppm.p2", pppm_ref_path, "Reference: PPPM 10000 passes", None, None, 0.0009, 0.003)
    ]:
    verifiers = [
...
def test_render(case):
    """
    A Cornell box with a top area light, normal-mapped reflective walls, and a
    normal-mapped glass sphere near the center of the box.
    """
